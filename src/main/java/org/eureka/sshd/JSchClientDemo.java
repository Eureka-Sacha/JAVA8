...
/**
 * @author: 奎
 * @date: 2017/9/17 17:33
 * @description:
 */
public class JSchClientDemo {
    public static void main(String[] args) throws JSchException, InterruptedException {
        JSch jSch = new JSch();
//        jSch.addIdentity("1234");
        Session session = jSch.getSession("eureka", "192.168.31.57", 22);
        session.setConfig("StrictHostKeyChecking", "no");
        session.setPassword("1234");
        session.connect();
        ChannelExec exec = (ChannelExec) session.openChannel("exec");
        exec.setCommand("ifconfig");
